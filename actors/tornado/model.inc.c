...
// VR comfort effect: reuse the original tornado texture and geometry, but
// render it white at 25% transparency (75% opacity). This display list is
// separate so the normal Tweester enemy keeps its original sand coloring.
// DynOS relocates display-list pointers in place during startup, so this
// nested command list must not be shared with the original Tweester model.
const Gfx vr_twirl_tornado_geometry_dl[] = {
    gsDPSetTextureImage(G_IM_FMT_IA, G_IM_SIZ_16b, 1, tornado_seg5_texture_05013128),
    gsDPLoadSync(),
    gsDPLoadBlock(G_TX_LOADTILE, 0, 0, 32 * 64 - 1, CALC_DXT(32, G_IM_SIZ_16b_BYTES)),
    gsSPLight(&tornado_seg5_lights_05014128.l, 1),
...
const Gfx vr_twirl_tornado_dl[] = {
    ...
    gsDPSetTile(G_IM_FMT_IA, G_IM_SIZ_16b, 8, 0, G_TX_RENDERTILE, 0, G_TX_WRAP | G_TX_NOMIRROR, 6, G_TX_NOLOD, G_TX_WRAP | G_TX_NOMIRROR, 5, G_TX_NOLOD),
    gsDPSetTileSize(0, 0, 0, (32 - 1) << G_TEXTURE_IMAGE_FRAC, (64 - 1) << G_TEXTURE_IMAGE_FRAC),
    gsSPDisplayList(vr_twirl_tornado_geometry_dl),
    gsSPTexture(0xFFFF, 0xFFFF, 0, G_TX_RENDERTILE, G_OFF),
    gsDPPipeSync(),
