...
static s32 envfx_interpolate_snow_y(
    ...
    // Snow vertices use integer world coordinates. Every normal flake falls
    // by the same small amount per 30 Hz tick, so ordinary truncation makes
    // the entire field cross its integer boundaries on the same render frame.
    // A stable per-particle phase distributes those sub-frame crossings while
    // preserving both exact simulation endpoints and deterministic gameplay.
    const f32 phase =
