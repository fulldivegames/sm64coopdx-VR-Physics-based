...
static u8 vr_painting_exit_hat_alpha(void) {
    ...
    const u32 elapsed = gGlobalTimer - sVrPaintingExitHatStartFrame;
    if (elapsed < VR_PAINTING_EXIT_HAT_HOLD_FRAMES) {
        return 255;
    }
    ...
    return (u8)clamp(
        255 - (s32)(fadeElapsed * 255 /
            VR_PAINTING_EXIT_HAT_FADE_FRAMES),
        0,
        255
    );
}
