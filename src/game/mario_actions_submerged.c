...
static s32 check_water_jump(struct MarioState *m) {
    ...
    if (vr_get_first_person_view_direction(m, viewDirection) &&
        ...
        // In first-person VR, forward means forward. Looking at least 45
        // degrees upward while pressing forward replaces the original,
        // counter-intuitive requirement to pull the stick backward.
