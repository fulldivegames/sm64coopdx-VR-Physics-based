...
static void vr_update_twirl_tornado_effect(struct MarioState *m) {
    ...
    // The Tweester mesh is much taller than a player at scale 1.0. Scale it
    // from the selected character's model-derived head height so the shell is
    // approximately 75% of that character's full model height.
...
static void vr_update_first_person_backpedal_state(
    ...
    if (!wasRequested &&
        ...
        // Keep the release movement and native skid, but bound the side-flip
        // opportunity. A held backward direction must transition into the
        // established reverse jog instead of sustaining the skid for seconds.
