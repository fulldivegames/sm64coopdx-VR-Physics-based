...
static bool djui_text_render(struct DjuiBase* base) {
    ...
    // display-list arena is exhausted, create_dl_translation_matrix()
    // can skip its push while the unconditional pop below still runs.
    // Keeping the root push/pop balanced prevents scrolling labels from
    // inheriting a world transform and appearing stretched or displaced.
