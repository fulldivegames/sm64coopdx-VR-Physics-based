...
void network_shutdown(bool sendLeaving, bool exiting, bool popup, bool reconnecting) {
    // A lobby/disconnect callback can arrive synchronously while the network
    // system is being torn down. Ignore that nested request so player, Lua,
    // and DJUI state is only destroyed once.
    if (sNetworkShutdownInProgress) {
        LOG_INFO("ignored nested network shutdown");
        return;
    }
