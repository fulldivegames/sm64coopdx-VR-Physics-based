...
void dynos_gfx_warmup_loaded_pack_shaders(void) {
    ...
    // Lua/DynOS .bin actors are registered in the valid-actor map even when
    // they are not part of a traditional pack directory.
    ...
    // Opaque and alpha are the two safe static variants shared by desktop GL
    // and Quest GLES. gfx_pc_precomp_shader also prepares their world and
    // light-map forms. More exotic Lua-created modes remain learned on first
    // use and persist through the normal runtime cache.
    u32 warmed = 0;
    for (const auto &combiner : combiners) {
        // Leave ample room in the fixed combiner pool for dynamic Lua modes.
        if (warmed >= 64) break;
