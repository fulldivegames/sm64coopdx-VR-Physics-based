...
static JNIEnv *quest_android_get_jni_env(bool *attached) {
    *attached = false;
    if (sActiveQuestApp == NULL ||
        sActiveQuestApp->android_app == NULL ||
        sActiveQuestApp->android_app->activity == NULL) {
        return NULL;
    }
    JavaVM *vm = sActiveQuestApp->android_app->activity->vm;
    JNIEnv *env = NULL;
...
static void quest_android_call_keyboard_method(
    const char *methodName, const char *initialText
) {
    bool attached = false;
    JNIEnv *env = quest_android_get_jni_env(&attached);
    if (env == NULL) return;
    jobject activity = sActiveQuestApp->android_app->activity->clazz;
    jclass activityClass = (*env)->GetObjectClass(env, activity);
    const char *signature = initialText != NULL
        ? "(Ljava/lang/String;)V" : "()V";
    jmethodID method = (*env)->GetMethodID(
        env, activityClass, methodName, signature);
    if (method != NULL) {
        if (initialText != NULL) {
            jstring text = (*env)->NewStringUTF(env, initialText);
            (*env)->CallVoidMethod(env, activity, method, text);
...
void quest_android_show_keyboard(const char *initialText) {
    quest_android_call_keyboard_method("showVrKeyboard",
        initialText != NULL ? initialText : "");
}

void quest_android_hide_keyboard(void) {
    quest_android_call_keyboard_method("hideVrKeyboard", NULL);
}

extern void djui_inputbox_android_queue_text(const char *text);
JNIEXPORT void JNICALL
Java_com_fulldivegames_sm64coopdxvr_QuestNativeActivity_nativeOnVrKeyboardText(
    JNIEnv *env, jclass clazz, jstring text
) {
    (void)clazz;
    const char *utf8 = text != NULL
        ? (*env)->GetStringUTFChars(env, text, NULL) : "";
    djui_inputbox_android_queue_text(utf8);
